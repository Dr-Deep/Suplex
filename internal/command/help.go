...
func NewHelpCommand(bot *internal.SuplexBot) *internal.Command {
	var cmd = &HelpCommand{SuplexBot: bot}
...
func (cmd *HelpCommand) Exec(s *discordgo.Session, ev *discordgo.Interaction) {
	...
	for _, cmd := range cmd.SuplexBot.CommandHandler.List() {
		var (
			...
			descr = cmd.ApplicationCommand.Description
		)
		fields = append(
			fields,
	...
	if err := s.InteractionRespond(
		...
		&discordgo.InteractionResponse{
			...
			Data: &discordgo.InteractionResponseData{
				Embeds: []*discordgo.MessageEmbed{help},
			},
		},
	); err != nil {
